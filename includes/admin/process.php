 ...
 * @return void
 */
function DLN_CS_save_options()
{

	if (!current_user_can('edit_theme_options')) {

		wp_die(__('You are not allowed to be on this page.', 'dln-coming-soon'));
	}

	if (
		!isset($_POST['DLN_CS_options_verify'])
		|| !wp_verify_nonce($_POST['DLN_CS_options_verify'], 'DLN_CS_save_options')
	) {

		wp_die(__('You are not allowed to access this page.', 'dln-coming-soon'));
	}

	$gfonts									= explode("|", DLN_CS_GFONTS);

	$options								= get_option('DLN_CS_options');
	...
	update_option('DLN_CS_options', $options);

	dilone_clear_cache();

	wp_redirect(admin_url('admin.php?page=' . DLN_CS_PLUGIN_SETTING_PAGE . '&status=1'));
}
 ...
 * @return void
 */
function DLN_CS_plugin_status($status)
{
